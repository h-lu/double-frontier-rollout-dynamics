...
这份文档用于整理双前沿推进模型背后的研究问题、数值路线和潜在论文方向。它更接近论文预备笔记，而不是面向用户的使用说明。
...
整个项目始终围绕三套嵌套系统展开，并且始终遵循“先低维、后高维”的研究顺序。
...
这是最先做筛查的层次，也是当前最重要的工作层。它主要用来判断：
...
这是第一次可能出现折叠奇点和 MMO 的层次，也是从平面动力学走向高维慢快几何的关键过渡模型。
...
当前项目使用的是“数值勘探参数盒”，不是经验校准。它的作用是帮助我们判断模型可能支持哪些动力学机制，而不是给出经验估计。
...
整个数值路线是分层推进的，核心思想是：先用最便宜、最稳健的实验拿下最重要的结论，再决定是否继续进入更困难的层次。
...
这些规则和实验本身同样重要，因为它们决定我们是否继续追一个方向，还是及时转向更稳妥的论文路线。
...
因此，当前仓库最适合被理解为一个已经成型的数值研究框架，而不是已经完成证明、定稿写作或完整论文封装的最终版本。
